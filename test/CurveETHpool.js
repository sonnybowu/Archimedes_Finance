...
describe("Test Pools Rewards", () => {
    ...
    describe("CurveFi Test", () => {
        it("CurveFi Test ETH pool", async () => {
            const dectUSD = 13;
            console.log(`Number of decimals in tUSD pool ${dectUSD}`);

            await curveFactory["deploy_plain_pool(string,string,address[4],uint256,uint256)"](
                "tUSD/ETH",
                "tUSDETH",
                [weth.address, tUSD.address, ethers.constants.AddressZero, ethers.constants.AddressZero],
                200,
                MainnetHelper.noExp(4000000)
            );

            let poolAddress = await curveFactory["find_pool_for_coins(address,address)"](weth.address, tUSD.address);
            let pool = await StableSwap.attach(poolAddress);
            ...
            await tUSD.approve(pool.address, MainnetHelper.noExp(10 ** 50));
            await weth.approve(pool.address, MainnetHelper.noExp(10 ** 50));

            let balanceBeforeCRV = await weth.balanceOf(adminWallet.address);
            let balanceBeforetUSD = await tUSD.balanceOf(adminWallet.address);
            let amt0 = 10 ** (6 + dectUSD) * 2;
            let amt1 = 10 ** (1 + 18) * 2;
            console.log("\n--------------------");
            ...
            await pool["add_liquidity(uint256[2],uint256)"]([MainnetHelper.noExp(amt1), MainnetHelper.noExp(amt0)], 0);
            ...
            await tUSD.connect(walletA).approve(pool.address, MainnetHelper.noExp(10 ** 50));
            //console.log(util.inspect(pool))
            await pool
                .connect(walletA)
                .functions["exchange(int128,int128,uint256,uint256)"](1, 0, MainnetHelper.noExp(balanceSend), 0);
